...
public class DogoldEntity extends BaseFootsoldierEntity {

    public DogoldEntity(EntityType<? extends BaseFootsoldierEntity> type, Level level) {
        super(type, level);
        NAME="dogold";

        this.setItemSlot(EquipmentSlot.MAINHAND, new ItemStack(KyoryugerItems.KENKA_JOUTOU.get()));
    ...
    protected void dropCustomDeathLoot(net.minecraft.server.level.ServerLevel serverLevel, net.minecraft.world.damagesource.DamageSource damageSource, boolean recentlyHit) {
        ...
        this.spawnAtLocation(new ItemStack(KyoryugerItems.EMPTY_ZYUDENCHI.get(), 5));
        this.spawnAtLocation(new ItemStack(KyoryugerItems.BLANK_DEBOTH_ZYUDENCHI.get(), 1));
        
        int rand = this.random.nextInt(5);
        if (rand == 0) {
            this.spawnAtLocation(KyoryugerItems.KENKA_JOUTOU.get());
        } else if (rand == 1) {
            this.spawnAtLocation(KyoryugerItems.PTERAGORDON_ZYUDENCHI.get());
        }
    }
}
