package com.liasdan.supersentaicraft.entity.summon;

import com.liasdan.supersentaicraft.SuperSentaiCraftCore;
import com.liasdan.supersentaicraft.entity.ai.RangedSwordgunAttackGoal;
   ...
   private ListTag REQUIRED_FORMS = new ListTag();
			...
			if (isTame()) {
        		if (p_28879_ instanceof Creeper || p_28879_ instanceof Ghast) {
        		    if (this.getMainHandItem().getItem() instanceof BowItem) {
        		    	...
        		    	|| this.getMainHandItem().is(ItemTags.create(ResourceLocation.fromNamespaceAndPath(SuperSentaiCraftCore.MODID, "arsenal/all_swordguns")))) || !this.getMeleeOnly();
					}
					return false;
				}
		   ...
		   this.updatePersistentAnger((ServerLevel)this.level(), true);
			if ( this.getOwner() instanceof Player owner && this.isAlive()) {
				...
				||owner.getItemBySlot(EquipmentSlot.FEET).getItem()!=this.getRequiredArmor(EquipmentSlot.FEET).getItem()) {
					...
					if (!this.REQUIRED_FORMS.isEmpty() && owner.getItemBySlot(EquipmentSlot.FEET).getItem() instanceof RangerChangerItem belt) {
						boolean formFound = false;
						if (!this.matchAllForms) for (int i = 0; i < belt.Num_Base_Form_Item; i++) {
							CompoundTag tag = new CompoundTag();
							tag.putInt("Slot", i + 1);
							tag.putString("Form", RangerChangerItem.get_Form_Item(owner.getItemBySlot(EquipmentSlot.FEET), i + 1).toString());
    ...
    public void reassessWeaponGoal() {
        if (!this.level().isClientSide) {
            ...
            if (itemstack.getItem() instanceof BowItem && !itemstack.is(ItemTags.create(ResourceLocation.fromNamespaceAndPath(SuperSentaiCraftCore.MODID, "arsenal/all_swordguns")))) {
	...
	public void setRequiredForms(Player player) {
	  if (this.getOwner() == player && player.getItemBySlot(EquipmentSlot.FEET).getItem() instanceof RangerChangerItem belt) {
		this.REQUIRED_FORMS.clear();
		for (int n = 0; n < belt.Num_Base_Form_Item; n++) addRequiredForm(RangerChangerItem.get_Form_Item(player.getItemBySlot(EquipmentSlot.FEET), n+1), n+1);
	  }
   }
	...
	public void addRequiredForm(RangerFormChangeItem formItem, int slot) {
